...
int f[maxN << 1], rk[maxN << 1], idx[maxM], from[maxM], to[maxM], w[maxM];
...
int main(){
    ...
    std::sort(idx, idx + m, [&](const int& a, const int& b){
        return w[a] > w[b];
    });
    for(int i = 0; i < m; i++){
        int& a = from[idx[i]], &b = to[idx[i]];
        if(find(a) == find(b)){
            std::cout << w[idx[i]];
            return 0;
        }
        merger(a, b + n), merger(a + n, b);
    }
    std::cout << 0;
    return 0;
}