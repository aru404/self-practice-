...
int kmp[maxN], idx[maxN], pre[maxN];

inline void earse(std::string& s1, std::string& s2){
    kmp[0] = -1;
    ...
    std::deque<int> q;
    for(int i = 0; i < n; i++){
        while(~idx[i] && s1[i] != s2[idx[i] + 1]) idx[i] = kmp[idx[i]];
        if(s1[i] == s2[idx[i] + 1]) ++idx[i];
        q.push_back(i);
        if(idx[i] == m - 1){
            for(int j = 0; j < m; j++) q.pop_back();
        }
        if(q.empty()) idx[i + 1] = -1;
        else idx[i + 1] = idx[q.back()];
    }

    while(!q.empty()) std::cout << s1[q.front()], q.pop_front();
}
