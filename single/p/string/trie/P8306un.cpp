...
public:
    int cnt, ed;
    char c;
    std::unique_ptr<std::vector<std::unique_ptr<TreeNode>>> sonNodes;

    TreeNode(char c = '#', int cnt = 0, int ed = 0):c(c), cnt(cnt), ed(ed){
        sonNodes = decltype(sonNodes)(new std::vector<std::unique_ptr<TreeNode>>(256, std::unique_ptr<TreeNode>(nullptr)));
