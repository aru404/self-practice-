...
struct node{
    ...
    node(bool isEnd = 0, bool cnt = 0):isEnd(isEnd), cnt(cnt){}
};
...
int main(){
    ...
    std::string s;
    std::cin >> n;
    for(int i = 0; i < n; i++) std::cin >> s, trie.ins(s);
    std::cin >> m;
    for(int i = 0; i < m; i++){
        std::cin >> s;
        auto it = trie.find(s);
        if(it == nullptr || !(it->isEnd)) std::cout << "WRONG" << '\n';
        else if(it->cnt) it->cnt = 0, std::cout << "OK" << '\n';
